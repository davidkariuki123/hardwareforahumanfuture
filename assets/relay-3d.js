...
if (canvas && device && window.WebGLRenderingContext) {
  ...
  scene.add(new THREE.HemisphereLight(0xfffbf2, 0x5a5146, 1.45));
  const key = new THREE.DirectionalLight(0xfff2dd, 3.0);
  key.position.set(-2.4, -1.7, 3.5);
  scene.add(key);
  const fill = new THREE.DirectionalLight(0xdce8ff, 1.65);
  ...
  function render(progress = currentProgress) {
    ...
    model.rotation.x = THREE.MathUtils.lerp(-0.56, 0, p);
  ...
  new GLTFLoader().load(
    './assets/models/relay-thin-authoritative.glb',
    (gltf) => {
      model = gltf.scene;
      scene.add(model);
